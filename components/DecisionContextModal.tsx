...
const getStateStyle = (state: string) => {
    const styles: Record<string, { bg: string; text: string; icon: string }> = {
        ...
        shadow: { bg: 'bg-slate-500/20', text: 'text-slate-400', icon: '👤' },
    };
    ...
    return (
        <>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                <div
                    className="bg-[#0a0a0a] border border-slate-700 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto"
                    onClick={(e) => e.stopPropagation()}
                >
                    {/* Header */}
                    <div className="bg-[#0d0d0d] border-b border-slate-800 px-6 py-4 flex items-center justify-between sticky top-0">
                        <div className="flex items-center gap-3">
                            <span className="text-xl">🔍</span>
                            <div>
                                <h2 className="text-lg font-bold text-white">Decision Context</h2>
                                {context && (
                    ...
                    {loading ? (
                        ...
                        <div className="p-6 space-y-6">
                            ...
                            <div className="bg-slate-800/30 rounded-xl p-4">
                                <div className="text-[10px] uppercase tracking-wider text-slate-500 mb-2">Target Keyword</div>
                            ...
                            <div className="bg-purple-900/20 border border-purple-800/30 rounded-xl p-4">
                                <div className="text-[10px] uppercase tracking-wider text-purple-400 mb-3">⚡ The Decision</div>
                                <div className="flex items-center gap-4 mb-3">
                                    ...
                                    <div className={`text-2xl ${context.bid.delta_pct > 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                                        →
                                    </div>
                                    <div>
                                        <div className="text-xs text-slate-500">AI Optimal</div>
                                        <div className="text-xl font-mono text-emerald-400">
                                            ${context.bid.optimal.toFixed(2)}
                                        </div>
                                    </div>
                                    <div className={`text-sm font-bold px-2 py-1 rounded ${context.bid.delta_pct > 0 ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'
                                        }`}>
                                        {context.bid.delta_pct > 0 ? '+' : ''}{context.bid.delta_pct.toFixed(1)}%
                                    </div>
                                </div>
                                {context.bid.reasoning && (
                                    <div className="text-sm text-slate-400 bg-slate-800/50 rounded p-3 font-mono">
                                        {context.bid.reasoning}
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div className="bg-slate-800/30 rounded-xl p-4">
                                    ...
                                    <div className="space-y-1 text-xs">
                                        <div className="flex justify-between">
                                            ...
                                            <span className="text-slate-300">{context.strategist.objective}</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span className="text-slate-500">Confidence</span>
                                            <span className="text-slate-300">{(context.strategist.confidence * 100).toFixed(0)}%</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="bg-slate-800/30 rounded-xl p-4">
                                    ...
                                    <div className="space-y-1 text-xs">
                                        <div className="flex justify-between">
                                            <span className="text-slate-500">M<sub>supply</sub></span>
                                            <span className={`font-mono ${context.sentinel.m_supply >= 0.9 ? 'text-emerald-400' :
                                                    context.sentinel.m_supply >= 0.5 ? 'text-amber-400' : 'text-red-400'
                                                }`}>
                                        ...
                                        <div className="flex justify-between">
                                            ...
                                            <span className="text-slate-300 font-mono">{context.sentinel.dos}d</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span className="text-slate-500">L<sub>eff</sub></span>
                                            <span className="text-slate-300 font-mono">{context.sentinel.l_eff}d</span>
                                        </div>
                                        {context.sentinel.is_braking && (
                                            <div className="text-red-400 mt-2 text-[10px]">
                                ...
                                <div className="bg-slate-800/30 rounded-xl p-4">
                                    ...
                                    <div className="text-xs text-slate-500 mb-3">discoveries active</div>
                            ...
                            <div className="bg-slate-800/20 border border-slate-700 rounded-xl p-4">
                                <div className="text-[10px] uppercase tracking-wider text-slate-500 mb-3">🔗 Causal Chain</div>
