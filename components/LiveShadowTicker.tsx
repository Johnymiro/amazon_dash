...
const getAgentInfo = (trigger: string | null, fsmState: string): { icon: string; name: string; color: string } => {
    if (trigger?.includes('PULSE') || trigger?.includes('AUTO')) {
        return { icon: '⚡', name: 'Pulse', color: 'text-purple-400' };
    }
    if (trigger?.includes('SENTINEL') || fsmState === 'rationing' || fsmState === 'defense') {
        return { icon: '🛡️', name: 'Sentinel', color: 'text-amber-400' };
    }
    if (trigger?.includes('STRATEGIST') || fsmState === 'launch') {
        return { icon: '🎯', name: 'Strategist', color: 'text-blue-400' };
    }
    // Default to Tactician
    return { icon: '🤖', name: 'Tactician', color: 'text-emerald-400' };
};
...
const getBidDirection = (current: number | null, optimal: number): { arrow: string; color: string; pct: string } => {
    if (!current || current === 0) {
        return { arrow: '→', color: 'text-slate-400', pct: 'N/A' };
    }

    const pctChange = ((optimal - current) / current) * 100;

    if (pctChange > 5) {
        return { arrow: '↑', color: 'text-emerald-400', pct: `+${pctChange.toFixed(0)}%` };
    } else if (pctChange < -5) {
        return { arrow: '↓', color: 'text-red-400', pct: `${pctChange.toFixed(0)}%` };
    }
    return { arrow: '→', color: 'text-slate-400', pct: `${pctChange.toFixed(0)}%` };
};
                ...
                >
                    {!expanded ? (
                        <div className="flex items-center gap-2">
                            <span className="text-slate-400 truncate block max-w-[350px]">
                                {logicTrace}
                            </span>
                            <span className="text-slate-600 text-[10px] group-hover:text-purple-400">▼</span>
                        </div>
                    ) : (
                        <div className="space-y-1 bg-slate-800/50 rounded p-2 -mx-1">
                            ...
                            <button
                                ...
                                className="mt-2 w-full text-center text-[10px] text-purple-400 hover:text-purple-300 bg-purple-500/10 hover:bg-purple-500/20 rounded py-1 transition-colors"
                            >
                                🔍 VIEW FULL CONTEXT
                            </button>
                            <div className="text-[10px] text-slate-600 text-right pt-1">▲ click to collapse</div>
                        </div>
                    )}
                </div>
            </td>
...
export default function LiveShadowTicker() {
    ...
    if (error) {
        return (
            <section className="bg-[#0a0a0a] border border-red-900/50 rounded-xl p-4">
    ...
    return (
        <section className="bg-[#0a0a0a] border border-slate-800 rounded-xl overflow-hidden">
            ...
            <div className="bg-[#0d0d0d] border-b border-slate-800 px-4 py-3 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-mono text-slate-300">LIVE SHADOW STREAM</span>
                    </div>
            ...
            <div
                ref={containerRef}
                className="overflow-x-auto overflow-y-auto max-h-[500px] scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent"
            >
            ...
            {/* Footer Stats Bar */}
            <div className="bg-[#0d0d0d] border-t border-slate-800 px-4 py-2 flex items-center justify-between text-xs font-mono text-slate-500">
                <div className="flex items-center gap-4">
